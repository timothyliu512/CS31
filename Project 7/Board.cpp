...
namespace cs31
{
    Board::Board() : mRoundCount( 0 ) // On first round
    {
    ...
    {
        if (mRoundCount < TOTALROUNDSALLOWED) // Uses mRound as index
        {
            moves[ mRoundCount ] = m;
            scores[ mRoundCount ] = s;
            // increment to the next round
            mRoundCount++; // Use it as index
        }
        else
        {
    ...
    Move Board::getMoveForRound( int round ) const
    {
        ...
        {
            throw std::logic_error("Exceeded # Rounds");
        }
        ...
        // for now...
        // Retrieve the move from the round of play
        // int round represents index of a round that's already been played
        
        // Check that round is in in bounds in this range [0,TOTALROUNDSALLOWED-1]
        // If not, throw logic_error
    ...
    Score Board::getScoreForRound( int round ) const
    {
        ...
        // Score s; //Only gets populated when you end the round
