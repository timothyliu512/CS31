...
//      create a Score object, add it to the Board, save it as the current score value and then return it
//      increment the Round value
Score Mastermind::endRound( Move m ) 
{
    
    // for now...
    // Create score object with move and mWinningMove
    // end round on board class
    // store score object as current score
    // increment round variable
    // return the score
    // Calling all the other functions across the project
...
{
    // for now...
    // return WIN if current score is perfect match
    // return LOSE if current round hits TOTALROUNDSALLOWED
    // otherwise still playing
    GAMEOUTCOME outcome = GAMENOTOVER;
    
    if ( mCurrentScore.isExactMatch() == true ) // Current score is perfect match
    {
        return GAMEWON;
    }
    
    if ( mRound == TOTALROUNDSALLOWED) // Exceeds total rounds allowed
    {
        return ( GAMELOST);
    }
